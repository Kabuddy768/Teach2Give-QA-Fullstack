 const users = [
    ...
    },
    {
      ...
      friends: [1, 3],
      posts: [
        { content: "Hiking in the Bay Area!", timestamp: "2025-06-12T14:20:00", likes: 12 },
  ...
  const analyzeUsers =(users)=>{
    ...
    const userPosts = users.filter(user =>user.posts.some(post => new Date(post.timestamp)>= oneWeekAgo))

    const popularPosts = userPosts.map(user => user.posts.filter(post => post.likes > 10))
    
    return userPosts,popularPosts
